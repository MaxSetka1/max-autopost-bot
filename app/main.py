...
from app.db import init_db, add_log, get_pending_posts, mark_post_sent
from app.content import make_content
from app.planner import poll_control

ROOT = Path(__file__).resolve().parents[1]
CFG_CH = ROOT / "config" / "channels.yaml"
CFG_SC = ROOT / "config" / "schedules.yaml"

CONTROL_POLL_SEC = int(os.getenv("CONTROL_POLL_SEC", "30"))
...
def job_send(alias: str, token_env: str, text: str, api_base: str | None = None):
    ...
    dry = not bool(token)  # если нет токена — DRY‑режим
...
def schedule_channel(ch: dict, slots: list, default_tz: str):
    ...
    for s in slots:
        ...
        # Пробный текст (на случай DRY‑поста или логов)
...
def main():
    ...
    # простой таймер для опроса control
    last_poll = 0
    while True:
        schedule.run_pending()

        now = time.time()
        if now - last_poll >= CONTROL_POLL_SEC:
            n = poll_control()
            if n:
                print(f"[CONTROL] processed {n} request(s)")
            last_poll = now
